...
var _ = Describe("NetworksConfig unit tests @unit", func() {
	...
	Describe("Verify order of importance for environment variables and config files", func() {
		...
		It("should load a specified file", func() {
			conf, err := config.LoadNetworksConfig(fmt.Sprintf(specifiedConfig, utils.ProjectRoot))
			Expect(err).ShouldNot(HaveOccurred())
		...
		It("should fail to load a bad file", func() {
			_, err := config.LoadNetworksConfig(fmt.Sprintf(badConfig, utils.ProjectRoot))
		...
		It("should load the config if we specify a Secret NetworksConfig type", func() {
			conf, err := config.LoadNetworksConfig(utils.ProjectRoot)
			Expect(err).ShouldNot(HaveOccurred())
			Expect(conf.KeepEnvironments).Should(Equal("Never"), "We either changed the default value in the config or it did not load correctly")
		})
