...
func ReadConfig() (*Config, error) {
	...
	if cfg.Network == nil {
		cfg.logger().Debug().Msgf("Network %s not found in TOML, trying to use URL", selectedNetwork)
		...
		for _, n := range cfg.Networks {
			if n.Name == DefaultNetworkName {
				...
				if selectedNetwork == "" {
					cfg.logger().Debug().Msg("No network name provided, using default network")
	...
	cfg.logger().Trace().Interface("Config", cfg).Msg("Parsed seth config")
	return cfg, nil
}
...
func (c *Config) Validate() error {
	...
	if c.Network.GasPriceEstimationEnabled {
		if c.Network.GasPriceEstimationBlocks == 0 {
			c.logger().Debug().Msg("Gas estimation is enabled, but block headers to use is set to 0. Will not use block congestion for gas estimation")
		}
