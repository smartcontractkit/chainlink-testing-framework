...
func WithPostgresDbDatabaseName(name string) PostgresDbOption {
	return func(c *PostgresDb) {
		if name != "" {
			c.DbName = name
		}
...
func WithPostgresDbUserName(name string) PostgresDbOption {
	return func(c *PostgresDb) {
		if name != "" {
			c.User = name
		}
...
func WithPostgresDbPassword(pass string) PostgresDbOption {
	return func(c *PostgresDb) {
		if pass != "" {
			c.Password = pass
		}
