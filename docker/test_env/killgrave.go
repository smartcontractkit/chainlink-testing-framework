...
func (k *Killgrave) StartContainer() error {
	...
	c, err := tc.GenericContainer(utils.TestContext(k.t), tc.GenericContainerRequest{
		ContainerRequest: cr,
		Started:          true,
		Reuse:            true,
		Logger:           l,
	...
	endpoint, err := GetEndpoint(utils.TestContext(k.t), c, "http")
	if err != nil {
		return err
	}
	k.Container = c
...
func (k *Killgrave) AddImposter(imposters []KillgraveImposter) error {
	...
	if k.Container != nil {
		...
		err = logWaitStrategy.WaitUntilReady(utils.TestContext(k.t), k.Container)
	}
	return err
}
