...
const (
	TXNodeLabel    = "geth-reorg-ethereum-geth"
	MinerNodeLabel = "geth-reorg-ethereum-miner-node"
)

// ReorgConfirmer reorg stats collecting struct
type ReorgConfirmer struct {
	env     *environment.Environment
	clients []client.BlockchainClient
...
func (rc *ReorgConfirmer) Verify() error {
	...
	for _, ab := range rc.altBlockNumbers[:len(rc.altBlockNumbers)-1] {
		...
		h, err := rc.clients[0].HeaderHashByNumber(context.Background(), big.NewInt(ab))
		if err != nil {
			return err
		}
		log.Info().
...
func (rc *ReorgConfirmer) forkNetwork(blk client.NodeBlock) error {
	...
	expName, err := rc.env.ApplyChaosExperiment(&experiments.NetworkPartition{
		FromMode:       "all",
		FromLabelKey:   "app",
...
func (rc *ReorgConfirmer) joinNetwork() error {
	return rc.env.StopChaosExperiment(rc.chaosExperimentName)
}
