...
func BuildBootstrapSpec(verifierAddr common.Address, chainID int64, fromBlock uint64, feedId [32]byte) *client.OCR2TaskJobSpec {
...
func BuildOCRSpec(
	...
	feedId [32]byte, bridges []client.BridgeTypeAttributes,
	csaPubKey string, msRemoteUrl string, msPubKey string,
	nodeOCRKey string, p2pV2Bootstrapper string, allowedFaults int) *client.OCR2TaskJobSpec {
...
ask_price [type=median allowedFaults={{.AllowedFaults}} index=2];
	...
	data := struct {
		Bridges       []client.BridgeTypeAttributes
		AllowedFaults int
	}{
		Bridges:       bridges,
...
func BuildBridges(eaUrls []*url.URL) []client.BridgeTypeAttributes {
	var bridges []client.BridgeTypeAttributes
	for _, url := range eaUrls {
		bridges = append(bridges, client.BridgeTypeAttributes{
