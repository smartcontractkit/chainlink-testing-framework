...
// ArbitrumMultinodeClient represents a multi-node, EVM compatible client for the Metis network
type ArbitrumMultinodeClient struct {
	*EthereumMultinodeClient
}

// ArbitrumClient represents a single node, EVM compatible client for the Metis network
type ArbitrumClient struct {
	*EthereumClient
}

// NewArbitrumClient returns an instantiated instance of the Metis client that has connected to the server
func NewArbitrumClient(networkSettings *config.ETHNetwork) (EVMClient, error) {
	client, err := NewEthereumClient(networkSettings)
	log.Info().Str("Network Name", client.GetNetworkName()).Msg("Using custom Arbitrum client")
	return &ArbitrumClient{client.(*EthereumClient)}, err
}

// NewArbitrumMultiNodeClient returns an instantiated instance of all Metis clients connected to all nodes
func NewArbitrumMultiNodeClient(
	_ string,
	...
	for idx, networkURL := range networkSettings.URLs {
		...
		ec, err := NewArbitrumClient(networkSettings)
		if err != nil {
			return nil, err
		}
		ec.SetID(idx)
	...
	return &ArbitrumMultinodeClient{multiNodeClient}, nil
}
...
func (m *ArbitrumClient) DeployContract(
	contractName string,
