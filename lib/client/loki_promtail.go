...
func (m *LokiLogWrapper) SetClient(c *LokiPromtailClient) {
	m.client = c
}
...
type LokiPromtailClient struct {
	logWrapper *LokiLogWrapper
	lokiClient.Client
}

// Handle handles adding a new label set and a message to the batch
func (m *LokiPromtailClient) Handle(ls model.LabelSet, t time.Time, s string) error {
...
func (m *LokiPromtailClient) HandleStruct(ls model.LabelSet, t time.Time, st interface{}) error {
...
func (m *LokiPromtailClient) StopNow() {
	m.Client.StopNow()
}
...
func NewLokiPromtailClient(extCfg *LokiConfig) (*LokiPromtailClient, error) {
