...
func (en *EthereumNetwork) generateGenesisAndFoldersIfNeeded(baseEthereumFork ethereum.Fork) (generatedDataHostDir, generatedDataContainerDir, valKeysDir string, err error) {
	...
	if !en.isRecreated {
		...
		valKeysGenerator, err = NewValKeysGeneretor(en.EthereumChainConfig, valKeysDir, append(en.getImageOverrideOpts(config.ContainerType_ValKeysGenerator), WithTestInstance(en.t))...)
		if err != nil {
		...
		genesis, err = NewEthGenesisGenerator(*en.EthereumChainConfig, generatedDataHostDir, baseEthereumFork, append(en.getImageOverrideOpts(config.ContainerType_GenesisGenerator), WithTestInstance(en.t))...)
		if err != nil {
