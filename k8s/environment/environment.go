...
type Config struct {
	...
	InsideK8s bool
	// ManifestUpdate will skip manifest updating when false upon connecting to the environment. Should be true if you wish to update the manifest (e.g. upgrade pods)
	ManifestUpdate bool
...
func New(cfg *Config) *Environment {
	...
	if targetCfg.JobImage != "" && !targetCfg.detachRunner && !targetCfg.ManifestUpdate {
...
func (m *Environment) RunCustomReadyConditions(customCheck *client.ReadyCheckData, podCount int) error {
	...
	if m.Cfg.JobImage != "" {
		...
		m.AddChart(NewRunner(&Props{
			...
			NoManifestUpdate:   m.Cfg.ManifestUpdate,
	...
	manifestUpdate := os.Getenv(config.EnvVarManifestUpdate)
	if manifestUpdate != "" {
		mu, err := strconv.ParseBool(manifestUpdate)
		if err != nil {
			return fmt.Errorf("manifest update should be bool: true, false")
		}
		m.Cfg.ManifestUpdate = mu
	}
	log.Debug().Bool("ManifestUpdate", m.Cfg.ManifestUpdate).Msg("Update mode")
	if m.Cfg.ManifestUpdate || m.Cfg.JobImage != "" {
