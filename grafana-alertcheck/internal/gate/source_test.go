...
func TestCheckGrafanaVersion(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.version, func(t *testing.T) {
			...
			if c.wantErr {
				require.Errorf(t, err, "CheckGrafanaVersion(%q)", c.version)
			} else {
				require.NoErrorf(t, err, "CheckGrafanaVersion(%q)", c.version)
			}
			for _, want := range c.wantContains {
				require.Contains(t, err.Error(), want,
					"must name both what was found and what is supported")
			}
		})
	}
}
...
func TestHTTPSource_Definitions_HappyPath(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "/api/ruler/grafana/api/v1/rules", r.URL.Path)
		require.Empty(t, r.URL.RawQuery, "Definitions reads the ruler API unfiltered")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write(body)
	}))
	defer srv.Close()
...
func TestHTTPSource_Skew(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.wantErr {
				require.Equal(t, int32(1), calls.Load(), "a skew hard error must never be retried")
			}
		})
	}
}
...
func TestHTTPSource_MissingDateHeader(t *testing.T) {
	...
	src := NewHTTPSource(srv.URL, "", clock)
	_, err := src.Version(context.Background())
	require.Error(t, err, "a missing Date header is a hard error")
	require.Equal(t, int32(1), calls.Load(), "a missing Date header must never be retried")
}
...
func TestHTTPSource_UnparseableDateHeader(t *testing.T) {
	...
	src := NewHTTPSource(srv.URL, "", clock)
	_, err := src.Version(context.Background())
	require.Error(t, err, "an unparseable Date header is a hard error")
	require.Equal(t, int32(1), calls.Load(), "an unparseable Date header must never be retried")
}
...
func TestHTTPSource_ObservationTiming(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			require.Equal(t, 4*time.Second, obs.Latency, "send through full body read — not just the 2s header round trip")
		})
	}
}
...
func TestHTTPSourceStalenessNeverFalsePositiveUnderSkew(t *testing.T) {
	...
	require.False(t, res.Unobservable,
		"100s behind Grafana's TRUE now is under the 120s limit — only a runner-clock leak (skewed +30s here) would push this over")
}
...
func TestHTTPSource_Retry_ExceedsLimit(t *testing.T) {
	...
	require.Equal(t, int32(6), calls.Load(),
		"maxSequentialFailures=5 tolerates 5, gives up on the 6th")
...
func assertRetryExhausted(t *testing.T, err error, wantFailures int) {
	...
	require.Equal(t, wantFailures, reErr.Failures)
	require.Contains(t, err.Error(), fmt.Sprintf("gave up after %d", wantFailures))
	_, ok := errors.AsType[*TransportError](err)
	require.False(t, ok, "an exhausted retry must be a terminal, non-retryable error")
}
...
func TestFakeClock(t *testing.T) {
	...
	require.True(t, c.Now().Equal(want))

	select {
	case fired := <-c.After(time.Hour):
		require.True(t, fired.Equal(want.Add(time.Hour)))
	default:
		require.Fail(t, "After(1h) did not fire immediately")
	}
}
